  ...
  return (
    <main className="reading-shell">
      <section className="reading-page">
        ...
        <div className="verdict-summary" aria-label="Resumen contable del trail"><span className="summary-approve">{verdictCounts.approve} aprobadas</span><span className="summary-escalate">{verdictCounts.escalate} escaladas</span><span className="summary-reject">{verdictCounts.reject} bloqueadas</span></div>
        <section className="audit-panel"><div className="panel-title"><span>EVENTOS · MÁS RECIENTES PRIMERO</span><small>{events.length} REGISTROS</small></div>
          {loading ? <p className="empty-copy">Consultando el trail de auditoría…</p> : events.length ? <div className="audit-table-wrap"><table className="audit-table"><thead><tr><th>Timestamp</th><th>Tipo</th><th>Mandato</th><th>Veredicto</th><th>Resumen</th></tr></thead><tbody>{events.map((event) => <tr className={`audit-row-${event.verdict?.toLowerCase() ?? "neutral"}`} key={event.event_id}><td>{formatDate(event.timestamp)}</td><td><span className="event-type">{event.type.replace(/_/g, " ")}</span></td><td><code>{event.mandate_id}</code></td><td>{event.verdict ? <span className={`verdict-tag verdict-${event.verdict.toLowerCase()}`}>{event.verdict}</span> : <span className="neutral-tag">—</span>}</td><td>{event.summary}</td></tr>)}</tbody></table></div> : <p className="empty-copy">Aún no hay actividad — corre a Saturday para empezar.</p>}
        </section>
      </section>
    </main>
  );
